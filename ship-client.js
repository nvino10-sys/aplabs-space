...
function setupShipSockets() {
  ...
  skt.on('ship:data', function(data) {
    shipData = data;
    window._shipMenuData = data; // expose for deepspace-client
  ...
  skt.on('ship:blueprintSold', function(data) {
    shipData.bars = data.bars;
    shipData.pendingBlueprint = null;
    SHIP_NOTIFY('Blueprint sold for ' + data.refund + ' bars');
    refreshTab();
  });
    ...
    if (shipMenuEl) {
      shipMenuEl.querySelectorAll('button').forEach(function(b) {
        if (b.textContent === 'ROLLING...' || b.textContent === '[ PROCESSING... ]') {
          b.disabled = false;
          b.style.opacity = '1';
          b.textContent = b.textContent === 'ROLLING...' ? 'ROLL RANDOM PART — ' + fmtNum(shipData.researchCost||250) + ' bars' : '[ SELL SHIP ]';
        }
      });
    }
  });
  ...
  skt.on('ship:scrapped', function(data) {
    ...
    refreshTab();
    SHIP_NOTIFY('Ship scrapped. +' + data.refund + ' bars returned.');
  });
  ...
  skt.on('ship:barsUpdate', function(data) {
    ...
    if (hdr) hdr.textContent = Math.floor(data.bars).toLocaleString() + ' bars';
  });
...
function showMainMenu() {
  ...
  titleWrap.innerHTML = '<span style="color:#00AAFF;font-size:' + (isMobileMenu?'13px':'18px') + ';font-weight:bold;letter-spacing:0.1em;white-space:nowrap;">SHIP SYSTEMS</span>'
    ...
    + '<span style="color:rgba(255,255,255,0.2);">|</span>'
    + '<span id="shipBarsHdr" style="color:#44FF88;white-space:nowrap;"><span id="ssBars"></span> bars</span>';
...
function refreshTab() {
  ...
  if (barsEl) barsEl.textContent = fmtNum(shipData.bars || 0);
  ...
  else if (activeTab === 'design')   buildDesignTab(ct);
}
...
function buildShipDetail(ct, ship) {
  ...
  // SHIP CANNOT FLY banner if no engine
  ...
  left.innerHTML = '<div style="display:flex;justify-content:space-between;margin-bottom:16px;">'
    + '<div><div style="font-size:36px;">'+(CLASS_ICONS[ship.class]||'?')+'</div>'
  ...
  if (!isActive) {
    ...
    ab.addEventListener('click', function() { SHIP_SKT()?.emit('ship:setActive', { shipId: ship.id }); });
    left.appendChild(ab);
  } else {
  ...
  scrapBtn.textContent = 'SCRAP SHIP (+'+_scrapRefund+' bars)';
  ...
  (function(sid,sname){scrapBtn.addEventListener('click',function(){
    if(!confirm('Scrap "'+sname+'"? You will receive '+_scrapRefund+' bars. All equipped parts are lost.')) return;
  ...
  SLOTS.forEach(function(slotId) {
    ...
    slbl.textContent = SLOT_ICONS[slotId] + ' ' + SLOT_LABELS[slotId].toUpperCase();
    ...
    if (part) {
      ...
      (function(sid, shipId) {
        uBtn.addEventListener('click', function(e) {
          e.stopPropagation();
          SHIP_SKT()?.emit('ship:unequip', { slot: sid, shipId: shipId });
        });
      })(slotId, ship.id);
  ...
  if (showEquip && activeShip) {
    ...
    (function(partId, slotId, shipId) {
      eBtn.addEventListener('click', function() {
        eBtn.disabled = true;
        SHIP_SKT()?.emit('ship:equip', { partId: partId, slot: slotId, shipId: shipId });
      });
...
function showTutorialGate(rollBtn, cost) {
  ...
  box.innerHTML = '<div style="font-size:36px;margin-bottom:12px;">⚠️</div>'
    ...
    + 'You might want that <b style="color:#44FF88;">' + fmtNum(cost) + ' bars</b> for something else.<br>'
...
function buildResearchTab(ct) {
  ...
  rollBtn.textContent = 'CRAFT RANDOM PART — ' + fmtNum(cost) + ' bars';
  ...
  balNote.textContent = canAfford ? ('Balance: ' + fmtNum(bars) + ' bars') : ('Need ' + fmtNum(cost-bars) + ' more bars');
  ...
  if (recipeList.length) {
    ...
    SLOT_ORDER.forEach(function(slotKey) {
      ...
      craftBtn.textContent = 'CRAFT — ' + fmtNum(cost) + ' bars';
      (function(key, btn) {
      ...
      unlearnBtn.textContent = 'UNLEARN (+50 bars)';
...
function buildDrawTab(ct) {
  ...
  if (drawReady && !_needsDesign) {
    ...
    drawBtn.addEventListener('click', function() { SHIP_SKT()?.emit('ship:claimDraw'); });
  ...
  if (bp) {
    ...
    craftBtn.textContent = 'BUILD -- ' + craftCost + ' bars' + (!canCraft ? '  (need ' + (craftCost-bars) + ' more)' : '');
    craftBtn.addEventListener('click', function() { if (canCraft) SHIP_SKT()?.emit('ship:craft'); });
