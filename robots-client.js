...
function updateAllRobots(delta, totalTime) {
  Object.values(allRobotMeshes).forEach(rm => {
    ...
    const cam = CAM();
    if (window._inStation) { rm.nameTag.style.display="none"; return; }
    if (!cam) return;
    const dx = cam.position.x - rm.group.position.x;
    const dz = cam.position.z - rm.group.position.z;
...
function setupSocketEvents() {
  ...
  skt.on('robots:state', data => {
    ...
    robotsBtn.style.display = (myRobots.length > 0 && !window._inStation && window._playerZone !== 'space') ? 'flex' : 'none';
  ...
  skt.on('robots:assembled', data => {
    ...
    window._myRobots = myRobots;
    if (!window._inStation && window._playerZone !== 'space') robotsBtn.style.display = 'flex';
...
function updateDepositBox(delta, totalTime) {
  const cam = CAM(); if (!cam) return;
  if (window._inStation) { depositHUD.style.display="none"; depositPrompt.style.display="none"; return; }
...
function updateBarsBox(delta, totalTime) {
  const cam = CAM(); if (!cam) return;
  if (window._inStation) { barsBoxHUD.style.display="none"; barsBoxPrompt.style.display="none"; return; }
