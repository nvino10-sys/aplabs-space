...
let _totalTime = 0;
function tick(delta) {
  if (window._playerZone === 'space') {
    atmPrompt.style.display = 'none';
    terminalPrompt.style.display = 'none';
    shadyPrompt.style.display = 'none';
    return;
  }
  _totalTime += delta;
  ...
  var _mkBlocked = window._inStation || window.dsActive || window._spaceOverride;
  atmPrompt.style.display      = (!_mkBlocked && !marketUIOpen && nearATM      && !nearTerminal) ? 'block' : 'none';
  terminalPrompt.style.display = (!_mkBlocked && !marketUIOpen && nearTerminal && !nearATM)      ? 'block' : 'none';
  shadyPrompt.style.display    = (!_mkBlocked && !marketUIOpen && nearShadyGuy && !nearATM && !nearTerminal) ? 'block' : 'none';
