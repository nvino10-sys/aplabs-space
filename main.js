...
function updateCosmo(delta) {
  if (window._inStation || window._spaceOverride) return;
  _cosmoT += delta;
...
document.addEventListener('keydown',e=>{
  ...
  if(e.code==="KeyE"&&!window._inStation) fireInteract();
...
function update(){
  ...
  if(!window._inStation) fountainSounds.forEach(fs=>{const dx=camera.position.x-fs.x,dz=camera.position.z-fs.z;fs.gain.gain.value=Math.max(0,0.10-Math.sqrt(dx*dx+dz*dz)*0.012);});
  animBlimp();
  if(!window._inStation) updateAnimals(delta, totalTime);
  updateRocket(delta, totalTime);
  ...
  if(!isSitting&&!golfMode&&!window._inStation){
    ...
    else if(nearGoldenTicket)       {doorPrompt.textContent='🎟️ Press E — Cosmo\'s Golden Tickets';doorPrompt.style.display='block';}
  ...
  if(!window._inStation) updateRobots(delta, totalTime);
  updateLounge(delta, totalTime);
  updatePet(delta, totalTime);
  updateNimbus(delta, totalTime);
  updateSlotProximity();
...
function sendPosition() {
  ...
  if(Math.sqrt(dx*dx+dz*dz)>0.05||dyaw>0.04||dy>0.05){
    if(!window._inStation&&!window._spaceOverride) socket.emit("player:move",{x:camera.position.x,y:camera.position.y,z:camera.position.z,rotY:yaw});
...
function fmtSB(n){ if(typeof n!=="number"||isNaN(n)) return "0"; if(n>=1e15){ var e=Math.floor(Math.log10(n)); return (n/Math.pow(10,e)).toFixed(2)+"e"+e; } if(n>=1e12) return (n/1e12).toFixed(1)+"T"; if(n>=1e9) return (n/1e9).toFixed(1)+"B"; if(n>=1e6) return (n/1e6).toFixed(1)+"M"; if(n>=1e3) return (n/1e3).toFixed(1)+"K"; return String(Math.floor(n)); }
// Notification toast
